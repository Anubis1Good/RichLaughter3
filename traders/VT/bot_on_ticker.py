...
bot_on_ticker = {
    ...
    'AFLT':(PEG17_PHOENIX,(None,29,29,12,54,42,6,0,55),3,None), #+1 +10
    'AFLT2':(PEG13_DWDDCr,(None,None,17,31,9),1,None), #+2
    'ALRS':(LEG2_DRINKER,(None,16,5,2.1,5,31,16,0),2,None), # +16 +0 -0
    'ALRS2':(UEG6_VULTURE,(None,18,51,42,18,6,3,3,0.81,12),2,None),#-2 -0

    'ASTR':(WEG3_BATYA,(None,40,21,2.1,1,1,1),4,None), #-4 +11 +8
    ...
    'MAGN':(PEG18_BLAZE,(None,None,43,10,54,38,20,0,55),1,None), #-3 +3 -3
    'MAGN2':(VEG1_VENUS,(7, 17, 0.5, 0.9, 0.1, 2.0, 0, 0, 0),2,None),#+10 +1 +1 -12 -1

    'MTLR':(PEG11_KUSURUKEN,(None,None,44,8,51,36,'c',55),1,None), #+9 +7
    # 'MTLR2':(LEG2_LYNX,(None,17,33,2.0,5,0.6,0),2,None), #-10 НБС
    'NLMK':(PEG18_BLAZE,(None,None,22,4,14,32,28,0,55),1,None), #-2 -6
    # 'NLMK2':(LEG2_LYNX,(None,None,4,1.8,2,1.4,0),1,None), #+1

    'RAGR':(UEG4_FALCON,(None,None,3,4,0.74),1,None), # -7
    'RAGR2':(UEG4_PELICAN,(None,None,3,2,0.34),1,None), #+6 -4
    'ROSN':(LEG2_FENNEC,(None,58,27,2.6,17,39,33,1.4,0),5,None), #-1 -2 +1
    'ROSN2':(PEG14_RENEGADE,(None,None,16,38,52,6,49,30),1,None), #+5

    'RUAL':(VEG1_VENUS,(None,None,0.3,1.0,0.1,0.5,0,1,1),1,None), #-6 -3
    'RUAL2':(LEG2_DRG,(None,None,12,2.7,4,15,11,1),1,None), #-3 +1
    'SBER':(PEG14_RENEGADE,(None,None,19,39,31,25,12,46),1,None), #+0
    'SBER2':(PEG4_U3,(112,None,53,23,2,55,'BB','mfi',4),10,None), #+1 +2 +2

    'SBERP':(PEG11_KUSURUKEN,(None,None,36,38,45,10,'c',55),1,None), #-1 -1 -1
    'SBERP2':(PEG26_UNKNOWN,(None,None,46,33,23,39,8,20,4,55,0),1,None), #+1
    'SFIN':(LEG2_DRINKER,(None,None,42,2.0,4,26,11,0),1,None), #+22 +6
    'SFIN2':(LEG1_PIN,(None,None,4,6,26,3),1,None), #+11

    # 'SGZH':(UEG6_DODO,(None,None,27,9,42,66),1,None),#-16 пока лучше не трогать
    # 'SGZH2':(PEG17_PHOENIX,(None,12,10,10,17,36,10,0,55),1,None), # пока лучше не трогать
    'SIBN':(WEG3_BATYA,(None,None,18,1.3,0,1,1),1,None), #+1 +3 +1
    # 'SIBN2':(LEG1_PHOGA,(None,None,2,1.1,18),1,None), #-6

    # 'SMLT':(VEG1_VENUS,(None,None,0.2,0.8,0.1,0.7,1,1,1),1,None), #+2 -11 пока лучше не трогать
    # 'SMLT2':(UEG6_PIGEON,(None,None,30,3,2,2,4,0.64,2.0,0),1,None), # -8 пока лучше не трогать
    'SMLT2':(SEG3_FORCE,(None,None,53,2.4,1,16,43,17,0.4),1,None), #+4
    'SNGSP':(LEG2_FENNEC,(None,None,9,2.5,2,31,19,1.8,1),1,None), #+1 +1 -1
    'SNGSP2':(WEG3_BATYA,(40, 48, 32, 2.9, 1, 1, 1),4,None), #+2 +3 +5 -3 +3


    'SPBE':(PEG11_KUSURUKEN,(None,None,53,8,29,37,'c',55),1,None), #+7 +2
    'SPBE2':(PEG18_BLAZE,(None,None,47,12,40,37,33,0,55),1,None), #+5 0 +6
    'T':(LEG2_LYNX,(None,None,53,1.5,32,0.5,0),1,None), #+0
    'T2':(PEG11_KUSURUKEN,(None,None,47,7,23,8,'c',55),1,None), #-3 -0 -2

    'TATN':(LEG1_PIN,(20,None,48,3,39,3),2,None), #+5 +9
    'TATN2':(WEG4_PUPPY,(None,None,41,39,11),1,None), #+7 +9 +4
    'TATNP':(LEG2_LYNX,(None,None,5,2.0,6,0.8,0),1,None), #-3 +3 -4
    'TATNP2':(SEG3_FORCE,(None,None,14,3.0,3,9,53,43,0.5),1,None), #+6 +2
    
    'VKCO':(PEG17_PHOENIX,(None,None,9,12,50,43,26,0,55),1,None), #+14 +10
    'VKCO2':(PEG11_KUSURUKEN,(None,None,21,13,47,20,'c',55),1,None), #+4 -7 +1
    'VTBR':(WEG3_BATYA,(None,None,15,2.0,1,1,1),1,None), #-3 +4 -4
    'VTBR2':(PEG4_UNIVERSAL,(None,None,23,13,12,36,'VG','s',6),1,None), #-2
