...
class BaseEG:
    ...
    def check_stop(self, pos, delta, action):
        ...
        if self.stop is not None and delta is not None:
            if delta * self.mult_ps <= -self.stop:
                # print(datetime.now(),self.symbol,delta * self.mult_ps,self.stop,'сработал стоп!')
                if pos > 0:
    ...
    def check_take(self, delta, action):
        if self.take is not None and delta is not None:
            if delta * self.mult_ps >= self.take:
                # print(datetime.now(),self.symbol,delta * self.mult_ps,self.take,'сработал тейк!')
    ...
    def check_valid_action(self, action: str, pos: int):
        ...
        if action is not None:
            if not self.can_long:
                if 'open' in action:
                    if 'long' in action:
                        # print(datetime.now(),self.symbol,'открытие лонга заблокированно')
                        return None
                    if 'all' in action:
            ...
            if not self.can_short:
                if 'open' in action:
                    if 'short' in action:
                        # print(datetime.now(),self.symbol,'открытие шорта заблокированно')
                        return None
                    if 'all' in action:
    ...
    def get_raw_action_fast(self, pdata_fast):
        """Быстрый режим"""
        df = pdata_fast['chart']
        idx = pdata_fast['idx']
        row = df.iloc[idx]
    ...
    def _get_action_from_row(self, row):
        ...
        Принимает строку (row) и полные данные (pdata, опционально).
        Переопределяется в каждой стратегии.
        """
        return None
    
    # pdata - processed data
    def __call__(self, pdata, pos, delta, *args, **kwds):
        # Проверяем, передан ли быстрый режим
        if isinstance(pdata, dict) and pdata.get('fast_mode', False):
            # Быстрый режим
            action = self.get_raw_action_fast(pdata)
        else:
            # Обычный режим
